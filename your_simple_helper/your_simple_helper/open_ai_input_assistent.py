...
def create_prompt_for_openai(command = dict, command_done = False):
    ...
    if command_done:
        ...
        if command.get('id') in simple_commands:
            ...
            return f'For {command.get("name")} that is {command.get("description")} you need this {command.get("arguments")}. Say this for user friendly form.'
        else:
            return f'Command {command.get("name")} with such description - {command.get("description")} was done. Tell this ifriendly to the user.'
